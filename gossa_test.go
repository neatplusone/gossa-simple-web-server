...
func doTestRegular(t *testing.T, url string, testExtra bool) {
	...
	nFiles, foundFile := getZip(t, "檔案.html", url+"zip?zipPath=%2F%E4%B8%AD%E6%96%87%2F&zipName=%E4%B8%AD%E6%96%87")
	if nFiles != 1 || !foundFile {
	...
	// ~~~~~~~~~~~~~~~~~
	fmt.Println("\r\n~~~~~~~~~~ test rpc robustness (bad args / unknown call / unknown hash)")
	if postJSON(t, url+"rpc", `{"call":"nope","args":[]}`) != `error` {
		t.Fatal("unknown rpc call didnt error")
	}
	if postJSON(t, url+"rpc", `{"call":"rm","args":[]}`) != `error` {
		t.Fatal("rm with missing args didnt error")
	}
	if postJSON(t, url+"rpc", `{"call":"sum","args":["/custom_mime_type.types","bogus"]}`) != `error` {
		t.Fatal("unknown hash algorithm didnt error")
	}
	// a valid sum must still return a hex digest (sha256 => 64 hex chars)
	body0 = postJSON(t, url+"rpc", `{"call":"sum","args":["/custom_mime_type.types","sha256"]}`)
	if len(body0) != 64 {
		t.Fatal("sha256 sum returned unexpected result: " + body0)
	}
	// server must still be responsive after the recovered errors above
	fetchAndTestDefault(t, url)
...
func TestAuth(t *testing.T) {
	...
	// correct credentials -> 200 with a real listing
	req, err = http.NewRequest("GET", base, nil)
	dieMaybe(t, err)
	req.SetBasicAuth("gossa", "secret")
	resp, err = http.DefaultClient.Do(req)
	dieMaybe(t, err)
	body, err := io.ReadAll(resp.Body)
	resp.Body.Close()
	dieMaybe(t, err)
	if resp.StatusCode != http.StatusOK || !strings.Contains(string(body), "<title>/</title>") {
		t.Fatalf("expected 200 listing with valid credentials, got %d", resp.StatusCode)
	}
}

func TestMaxUpload(t *testing.T) {
	fmt.Println("========== testing max upload limit ============")
	base := "http://127.0.0.1:8000/"

	// an upload well under the 1MB limit should still succeed
	if small := postDummyFile(t, base, "maxupload-small.txt", "hello"); small != "ok" {
		t.Fatal("small upload under the limit should succeed, got: " + small)
	}

	// a >1MB payload must be rejected by MaxBytesReader
	big := strings.Repeat("A", 2*1024*1024)
	if res := postDummyFile(t, base, "maxupload-big.txt", big); res == "ok" {
		t.Fatal("upload above the limit should have been rejected")
	}

	// cleanup any files created above so the fixture stays pristine
	postJSON(t, base+"rpc", `{"call":"rm","args":["/maxupload-small.txt"]}`)
	postJSON(t, base+"rpc", `{"call":"rm","args":["/maxupload-big.txt"]}`)
}
